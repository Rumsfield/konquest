    ...
    public void execute() {
    	...
    	if (getArgs().length != 2) {
        	...
        	if(getKonquest().getKingdomManager().isChunkClaimed(playerLoc.getChunk())) {
        		...
        		ChatUtil.sendNotice((Player) getSender(), MessagePath.COMMAND_ADMIN_SETTRAVEL_NOTICE_SUCCESS.getMessage(getKonquest().getKingdomManager().getChunkTerritory(playerLoc.getChunk()).getName()));
        	} else {
